        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {categories.map((category, index) => (
            <div
              ...
              className={`group animate-fade-in-up delay-${index * 150}`}
              style={{ animationDelay: `${index * 150}ms` }}
            >
              <Link
                to={`/category/${category.category}`}
                className="block bg-white dark:bg-slate-900/90 dark:bg-slate-800/90 backdrop-blur-sm p-8 rounded-2xl border border-slate-200 dark:border-slate-800/50 dark:border-slate-700/50 hover:border-primary/50 transition-all duration-300 hover:shadow-2xl hover:shadow-primary/10 hover:scale-105"
              >
